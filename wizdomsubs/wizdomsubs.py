...
myVersion = "0.1.0"
myTmp = "C:\\.wizdomsubs"
mySubFolder = path.join(myTmp, 'subs')
...
def download(id):
    ...
    # TODO: Extract archive_file to mySubFolder
    for file_ in os.listdir(mySubFolder)[1]:
...
def searchTMDB(type, query, year):
    ...
    filename = 'wizdom.search.tmdb.%s.%s.%s.json' % (
        type, lowercase_with_underscores(query), year)
    if year > 0:
    ...
    url = "http://api.tmdb.org/3/%s/%s/external_ids?api_key=%s&language=en" % (
        type, tmdb_id, tmdbKey)
    response = get(url)
...
def wlog(msg):
    print((u"##**## [%s] %s" % ("Wizdom Subs", msg,)).encode('utf-8'))


# ---- main -----
if __name__ == "__main__":
    if not os.exists(myTmp):
        os.makedirs(myTmp)
    ...
    if action == 'search':
        ...
        if imdb_id[:2] == "tt":  # Simple IMDB_ID
            searchByIMDB(imdb_id, item['season'],
                         item['episode'], item['file_original_path'])
        else:
            ...
            # Search Movie by Title+Year
                ...
                try:
                    ...
                    wlog('Caught Exception: error in movie search: %s' %
                         format(err))
                    pass
        ...
        if debug:
            if imdb_id[:2] == "tt":
                print("Debug "+myVersion, str(item), "imdb: "+str(imdb_id))
            else:
                print("Debug "+myVersion, str(item), "NO IDS")
        ...
        try:
            response = get("https://subs2.apollogroup.tv/kodi.upload.php?status=1&imdb=%s&season=%s&episode=%s" %
                           (getParam("imdb", params), getParam("season", params), getParam("episode", params)))
            ap_object = loads(response.text)["result"]
            if "Hebrew" not in ap_object["lang"]:
                response = get("https://subs2.apollogroup.tv/kodi.upload.php?upload=1&lang=he&subid=%s&imdb=%s&season=%s&episode=%s" %
                               (getParam("id", params), getParam("imdb", params), getParam("season", params), getParam("episode", params)))
                ap_upload = loads(response.text)["result"]
                if "error" in ap_upload:
                    print("Apollo Error", "%s" % ap_upload["error"])
                else:
                    print("Apollo", "Sub uploaded. Thank you!")
        except:
            pass
